...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (!IsBlackListEmpty) {
      if (userData) {
        // Split the string into an array to check if the user country is in the blacklist
        const blacklistCountries = process.env.NEXT_PUBLIC_BLACKLIST_COUNTRIES.split(',');
  
        // Check if the user country is in the blacklist
        if (blacklistCountries.includes(userData.country)) {
      ...
      {!isBlackListed ? (
        <div className="relative snap-mandatory min-h-screen bg-AAprimary w-full ">
          ...
          {context.sharedState.finishedLoading ? (
            <Footer githubUrl={"https://github.com/souvikmsd7"} hideSocialsInDesktop={true} />
          ) : (
            <></>
          )}
